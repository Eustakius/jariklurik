...
class User extends \Myth\Auth\Entities\User
{
    ...
    public function getRolesSet(): array
    {
        $groupModel = new GroupModel();
        $groups = $groupModel->findAll();
        
        $userGroups = !isset($this->id) ? [] : array_column(model(GroupModel::class)->getGroupsForUser($this->id), 'group_id') ;
        
        return array_map(function ($group) use($userGroups) {
            return [
                'value' => $group->id,
                'label' => $group->name,
                'checked' => in_array($group->id, $userGroups),
            ];
        }, $groups);
    }
    /**
    ...
    public function getCompanyRelation()
    {
        ...
        $company = $companyModel->where('user_id', $this->id)->first();
        if(!empty($company)){
            return $company;
        }
        return null;
    ...
    public function formatDataTableModel()
    {
        ...
        return [
            ...
            'created_at' => isset($this->created_at) ? Time::parse($this->created_at)->format('Y-m-d H:i:s') : '',
            'updated_at' => isset($this->updated_at) ? Time::parse($this->updated_at)->format('Y-m-d H:i:s') : '',
            'deleted_at' => isset($this->deleted_at) ? Time::parse($this->deleted_at)->format('Y-m-d H:i:s') : '',
            'status' => statusRender($this->status),
            'active' => statusRender($this->active),
            'user_type' => ucfirst($this->user_type),
        ];
    }

...
$(function () {
    ...
    $(window).on('scroll', function () {
        ...
        const opacity = Math.max(0, 1 - $(this).scrollTop() / maxScroll);
        $fadeDiv.css('opacity', opacity);
    });

    // Toggle fade-in / fade-out
    $btn.on('click', function () {
        if ($fadeJob.hasClass('opacity-0')) {
            $(this).text('Apply Loker');
            // Munculkan box
            $fadeJobApply.hide();
            // sedikit delay agar transition aktif
            setTimeout(() => {
                $fadeJob.show(); // tampilkan dulu
                $fadeJob.removeClass('opacity-0').addClass('opacity-100');
                // Hilangkan box
                // tunggu transisi selesai (500ms sesuai Tailwind)
                setTimeout(() => {
            ...
            setTimeout(() => {
                $fadeJob.hide();
                $fadeJobApply.show(); // tampilkan dulu
                // sedikit delay agar transition aktif
                setTimeout(() => {
